# EcoSynTech PCB v6.3 Final — PCB Layout Guide V3 (FINAL)
# ==============================================================
# 2-Layer 200mm x 150mm | ESP32-WROOM-32E | Final per original spec
# ==============================================================

## FABRICATION SPECIFICATIONS (per spec H1)

- Material: FR-4, Tg ≥ 130°C
- Layers: 2 (Top + Bottom)
- Copper weight: **1 oz TOP, 2 oz BOTTOM** (2 oz bottom mandatory under relay zone)
- **Surface finish: ENIG** (mandatory per spec H1)
...
Center-Left (X:5-100, Y:65-115):   Z5: ESP32 Module + MicroSD + Status LEDs + BOOT_OK
...
### POWER_GOOD + RELAY_EN Layout:
- U_PWR_GOOD (MCP809T-315): Place near +3V3_ESP rail entry, close to ESP32
- C_PWR_GOOD: Right at VCC pin of U_PWR_GOOD
- R_LED_PWR: Between POWER_GOOD and LED_PWR
- D_RELAY_PWR_GOOD, D_RELAY_BOOT_OK: Place near relay driver section
- R_RELAY_EN_PU, R_RELAY_EN_PD: Place near ESP32 (RELAY_EN is firmware GPIO)
- TP_POWER_GOOD, TP_RELAY_EN, TP_BOOT_OK: Test points near their respective circuits

### TPL5010 Watchdog Layout:
- R_WD_RT (49.9kΩ): Place close to TPL5010 RT pin, short trace to GND
- C_WD_FILT: Near VCC pin
- D_RST (BAT54S): Near EN_ESP pin, cathode to EN_ESP, anode to WATCHDOG_RST
- TP_WD_RST: Near D_RST
...
BOOT_OK signal: From ESP32 IO1, routed to relay interlock circuit
...
## SILKSCREEN REQUIREMENTS (per spec E)
...
POWER    ← LED (driven by POWER_GOOD)
WIFI     ← LED
MQTT     ← LED
ERROR    ← LED
...
| TP_POWER_GOOD | POWER_GOOD | Near MCP809T-315 |
| TP_RELAY_EN | RELAY_EN | Near relay enable circuit |
| TP_BOOT_OK | BOOT_OK | Near ESP32 |
| TP_WD_RST | WATCHDOG_RST | Near BAT54S D_RST |
